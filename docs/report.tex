...
This query shows the number of games won in a given period of the game.

\textbf{Business Value:}

The analysis helps to determine how often the game ends in different periods of the game. We see that the game ends almost 2 times more often at the end than in the middle of the game, and 5 times more often than in the opening.
...
\begin{itemize}
    \item \texttt{numTrees}: [50, 100, 150]
    \item \texttt{maxDepth}: [5, 10, 15]
    \item \texttt{minInstancesPerNode}: [1, 5, 10]
\end{itemize}

Total combinations: 27

\subsubsection{Model 2: Support Vector Machine (SVM)}

Hyperparameters tuned:

\begin{itemize}
    \item \texttt{regParam}: [0.01, 0.1, 1.0]
    \item \texttt{maxIter}: [50, 100, 150]
    \item \texttt{aggregationDepth}: [2, 3, 4]
\end{itemize}
...
\begin{itemize}
    \item \texttt{smoothing}: [0.5, 1.0, 1.5]
    \item \texttt{modelType}: [multinomial, bernoulli, gaussian]
    \item \texttt{threshold}: [0.3, 0.5, 0.7]
\end{itemize}
...
\midrule
Dashboard creation & 5\% & 70\% & 20\% & 5\% & dashboard & 5 \\
\midrule
Report writing & 15\% & 20\% & 50\% & 15\% & report.pdf & 10 \\
\midrule
Presentation & 20\% & 10\% & 60\% & 10\% & slides.pdf & 4 \\
\bottomrule
\end{tabular}
